...
% %%%%%%%%
% UseManuallyCorrectedPositionFiles = questdlg('Base acquition on files in ManuallyCorrectedStagePositionsDirectory?', 'Question', 'Yes', 'No', 'No');
% 
% if strcmp(UseManuallyCorrectedPositionFiles, 'No')
%     UseImagedBasedStageCorrection = questdlg('Do you want to use image based stage correction?', 'Question', 'Yes', 'No', 'No');
% else
%     UseImagedBasedStageCorrection = 'No';
% end
% 
% if strcmp(UseImagedBasedStageCorrection, 'Yes')
%     PerformAutoFocusDuringIBSC = questdlg('Do you want to perform an Auto Focus during IBSC?', 'Question', 'Yes', 'No', 'No');
% end

%% Get some Montage settigns
PerformAutoFocusDuringIBSC = GuiGlobalsStruct.MontageParameters.IBSC_AutoFoc;
UseImagedBasedStageCorrection = GuiGlobalsStruct.MontageParameters.IBSC_Do;
UseManuallyCorrectedPositionFiles = GuiGlobalsStruct.MontageParameters.UseManuallyCorrectedPositionFiles;
...
dirname = GuiGlobalsStruct.TempImagesDirectory;
if isempty(dirname)
    ...
    GuiGlobalsStruct.TempImagesDirectory = dirname;
end
...
if ~UseManuallyCorrectedPositionFiles
...
GuiGlobalsStruct.MontageParameters.AFLastWD = GuiGlobalsStruct.MontageParameters.AFStartingWD;



%% Walk through all sections

%ArrayOfSectionIndexes = waferProgress.manualRetakeList;
...
%     end
    ...
    if AreThereAnyImagesToTakeOnThisSection | doManualRetake
        ...
        if ~UseManuallyCorrectedPositionFiles
            if UseImagedBasedStageCorrection
                if PerformAutoFocusDuringIBSC
