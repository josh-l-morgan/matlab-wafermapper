...
% Last Modified by GUIDE v2.5 16-Dec-2022 18:14:04
...
set(handles.r_offset_Slider, 'Max', -AlignOverviewsGuiGlobalStruct.r_offset+200);
set(handles.r_offset_Slider, 'Min', -AlignOverviewsGuiGlobalStruct.r_offset-200);

set(handles.c_offset_Slider, 'Value', AlignOverviewsGuiGlobalStruct.c_offset);
set(handles.c_offset_Slider, 'Max', AlignOverviewsGuiGlobalStruct.c_offset+200);
set(handles.c_offset_Slider, 'Min', AlignOverviewsGuiGlobalStruct.c_offset-200);

set(handles.r_coarseOffset_Slider, 'Value', -AlignOverviewsGuiGlobalStruct.r_offset);
set(handles.r_coarseOffset_Slider, 'Min', -AlignOverviewsGuiGlobalStruct.r_offset-3000);
set(handles.r_coarseOffset_Slider, 'Max', -AlignOverviewsGuiGlobalStruct.r_offset+3000);

set(handles.c_coarseOffset_Slider, 'Value', AlignOverviewsGuiGlobalStruct.c_offset);
set(handles.c_coarseOffset_Slider, 'Min', AlignOverviewsGuiGlobalStruct.c_offset-3000);
set(handles.c_coarseOffset_Slider, 'Max', AlignOverviewsGuiGlobalStruct.c_offset+3000);
...
if 1 %color
    ...
    if isfield(AlignOverviewsGuiGlobalStruct,'zoomWindow')
        if ~AlignOverviewsGuiGlobalStruct.zoomWindow
            imshow(ColorCombinedImage,'InitialMagnification', 'fit');
        else
            zoomWindow = round(AlignOverviewsGuiGlobalStruct.zoomWindow);
            %zoomWindow(:,2) = size(ColorCombinedImage,1)-zoomWindow(:,2)+1;
            imshow(ColorCombinedImage(zoomWindow(1,2):zoomWindow(2,2),...
                zoomWindow(1,1):zoomWindow(2,1),:));
        end
    else
        imshow(ColorCombinedImage,'InitialMagnification', 'fit');
    end
...
AlignOverviewsGuiGlobalStruct.OverviewImage(AlignOverviewsGuiGlobalStruct.OverviewImage == 0) = 1;
...
if 1
    ...
    oY1 = max(1,oY1); nY1 = max(1,nY1);
    oY2 = min(MaxR,oY2); nY2 = min(MaxR,nY2);
    ...
    oX1 = max(1,oX1); nX1 = max(1,nX1);
    oX2 = min(MaxC,oX2); nX2 = min(MaxC,nX2);
    
    OverviewImage_rotated_shifted(nY1:nY2,nX1:nX2) = OverviewImage_rotated(oY1:oY2,oX1:oX2);
    
else
    OverviewImage_rotated_shifted = OverviewImage_rotated * 0 ;
    [y x] = find(OverviewImage_rotated_shifted+1);
    ny = y + r_offset;
    nx = x + c_offset;
    nUse = (ny>0) & ( ny<=MaxR) & ( nx > 0) & ( nx <= MaxC);
    nInd = sub2ind([MaxC MaxR],ny(nUse),nx(nUse));
    OverviewImage_rotated_shifted(nInd) = OverviewImage_rotated(nUse);
...
if 1
    ...
    T = AlignOverviewsGuiGlobalStruct.SectionOverviewTemplateCroppedFilled;
    tempFig = figure;
   CC = cat(3,T, OverviewImage_rotated, OverviewImage_rotated_shifted);
   image(uint8(CC)),%
   pause(1)
   close(tempFig)
end
...
%fill in all transform produced border pixels with original background average
...
fprintf('r_offset_Slider = %d\n', r_offset);

set(handles.r_offset_EditBox,'String',num2str(r_offset));


set(handles.r_coarseOffset_Slider, 'Value', -r_offset);
set(handles.r_coarseOffset_Slider, 'Min', -r_offset-3000);
set(handles.r_coarseOffset_Slider, 'Max', -r_offset+3000);
...
fprintf('c_offset_Slider = %d\n', c_offset);

set(handles.c_offset_EditBox,'String',num2str(c_offset));

set(handles.c_coarseOffset_Slider, 'Value', c_offset);
set(handles.c_coarseOffset_Slider, 'Min', c_offset-3000);
set(handles.c_coarseOffset_Slider, 'Max', c_offset+3000);
...
set(handles.AngleOffset_EditBox,'String',num2str(0));
set(handles.AngleOffset_Slider, 'Value',0); %note minus sign


set(handles.c_offset_EditBox,'String',num2str(0));
set(handles.c_offset_Slider,'Value', 0); %note minus sign



set(handles.r_offset_EditBox,'String',num2str(0));
set(handles.r_offset_Slider, 'Value',0);
...
% --- Executes on slider movement.
function r_coarseOffset_Slider_Callback(hObject, eventdata, handles)
% hObject    handle to r_coarseOffset_Slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
r_offset = -get(hObject, 'Value'); %note minus sign
fprintf('r_offset_Slider = %d\n', r_offset);

set(handles.r_offset_EditBox,'String',num2str(r_offset));

set(handles.r_offset_Slider, 'Value', -r_offset);
set(handles.r_offset_Slider, 'Max', -r_offset+200);
set(handles.r_offset_Slider, 'Min', -r_offset-200);


ApplyTransform(handles);

% --- Executes during object creation, after setting all properties.
function r_coarseOffset_Slider_CreateFcn(hObject, eventdata, handles)
% hObject    handle to r_coarseOffset_Slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on slider movement.
function c_coarseOffset_Slider_Callback(hObject, eventdata, handles)
% hObject    handle to c_coarseOffset_Slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'Value') returns position of slider
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
c_offset = get(hObject, 'Value'); %note minus sign
MyStr = sprintf('c_offset_Slider = %d', c_offset);
disp(MyStr);

set(handles.c_offset_EditBox,'String',num2str(c_offset));

set(handles.c_offset_Slider, 'Max', c_offset+200);
set(handles.c_offset_Slider, 'Min', c_offset-200);
set(handles.c_offset_Slider, 'Value', c_offset);


ApplyTransform(handles);

% --- Executes during object creation, after setting all properties.
function c_coarseOffset_Slider_CreateFcn(hObject, eventdata, handles)
% hObject    handle to c_coarseOffset_Slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on slider movement.
function fineRotation_Callback(hObject, eventdata, handles)
% hObject    handle to fineRotation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function fineRotation_CreateFcn(hObject, eventdata, handles)
% hObject    handle to fineRotation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in nudgeRotationCounterClockwise.
function nudgeRotationCounterClockwise_Callback(hObject, eventdata, handles)
% hObject    handle to nudgeRotationCounterClockwise (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
AngleOffset = get(handles.AngleOffset_Slider, 'Value'); %note minus sign
AngleOffset = AngleOffset + .1;
set(handles.AngleOffset_Slider, 'Value', AngleOffset);
...
% --- Executes on button press in nudgeRotationClockwise.
function nudgeRotationClockwise_Callback(hObject, eventdata, handles)
% hObject    handle to nudgeRotationClockwise (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
AngleOffset = get(handles.AngleOffset_Slider, 'Value'); %note minus sign
AngleOffset = AngleOffset - .1;
...
% --- Executes on button press in makeZoomBox.
function makeZoomBox_Callback(hObject, eventdata, handles)
% hObject    handle to makeZoomBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global  AlignOverviewsGuiGlobalStruct;

AlignOverviewsGuiGlobalStruct.zoomWindow = 0;
DisplayColorCombinedImage(handles);

disp('mouse click opposite corners and press enter')
pts = ginput;
AlignOverviewsGuiGlobalStruct.zoomWindow = [min(pts); max(pts)];
AlignOverviewsGuiGlobalStruct.zoomWindow
DisplayColorCombinedImage(handles);
