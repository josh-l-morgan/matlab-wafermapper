function[] = AlignOverviews_CrossCorrelationLong()
...
%     
    ...
    B_gaussian = fspecial('gaussian',200,20);
   % ReImagedImageDSW = imfilter(ReImagedImageDS,B_gaussian,'symmetric');
    ReImagedImageDSN = mexHatSection(ReImagedImageDS);
    
    contrastI = imfilter(abs(ReImagedImageDSN),B_gaussian);
    contrastI = contrastI/max(contrastI(:));
    ReImagedImageDSN = ReImagedImageDSN./contrastI;
   
    
    ReImagedImageDS = ReImagedImageDSN;%ReImagedImageDSW.^2 .* ReImagedImageDSN;
    ReImagedImageDS = ReImagedImageDS * 256/max(ReImagedImageDS(:));

    image(ReImagedImageDS * 2)
...
%     
    ...
    OverviewImage_rotated_shifted(nY1:nY2,nX1:nX2) = OverviewImage_rotated(oY1:oY2,oX1:oX2);
       ...
       [y x] = find(OverviewImage_rotated*0+1);
       ny = y + r_offset;
       nx = x + c_offset;
       nUse = (ny>0) & ( ny<=MaxR) & ( nx > 0) & ( nx <= MaxC);
       nInd = sub2ind([MaxC maxR],ny,nx);
       OverviewImage_rotated_shifted(nInd(nUse)) = OverviewImage_rotated(nUse);
