...
global GuiGlobalsStruct globTPA
global AlignTargetPointListGuiGlobalStruct;
...
elseif (WaferNameIndex-1) >= 1
    ...
    WaferName = WaferNameList{WaferNameIndex};
    MaxSectionNumInThisWafer = length(AlignTargetPointListGuiGlobalStruct.AlignedTargetList.WaferArray(WaferNameIndex).SectionArray);
...
PreviousImageFileNameStr = sprintf('%s\\LowResAligned_%s_Section_%s.tif', AlignTargetPointListGuiGlobalStruct.AlignedTargetListDir, WaferName, num2str(SectionNum));
%fprintf('PreviousImageFileNameStr = %s\n',PreviousImageFileNameStr);
...
%%Zoom
[ys xs zs] = size(ColorCombinedImage);
ys2 = (ys-1)/globTPA.zoom/2;
xs2 = (xs-1)/globTPA.zoom/2;
CC = ColorCombinedImage(round((ys+1)/2-ys2):round((ys+1)/2+ys2),...
    round((xs+1)/2-xs2):round((xs+1)/2+xs2),:);



imshow(CC);
[r_max, c_max] = size(CC(:,:,1));
...
ImageFileNameStrR = AlignedTargetList.WaferArray(WaferNameIndex).SectionArray(SectionIndex).ImageFileNameStr;
WafDir = GuiGlobalsStruct.WaferDirectory;
ImageFileNameStrR = regexprep(ImageFileNameStrR,'//','/');
ImageFileNameStrR = regexprep(ImageFileNameStrR,'/','\');%pratyush made changge for mix of / and \
prev = regexp(ImageFileNameStrR,'\');
if isempty(prev)
    ImageFileNameStrR = regexprep(ImageFileNameStrR,'//','/');%pratyush made changge to make ignore //
    prev = regexp(ImageFileNameStrR,'/');
end
ImageFileNameStr = [WafDir ImageFileNameStrR(prev(end-1):end)];
...
% --- Executes on selection change in popupmenu_zoom.
function popupmenu_zoom_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenu_zoom (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popupmenu_zoom contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_zoom

global globTPA
strs = get(hObject,'String');
val = get(hObject,'Value');
globTPA.zoom = str2num(strs{val}); 
UpdateDisplay(handles)

% --- Executes during object creation, after setting all properties.
function popupmenu_zoom_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenu_zoom (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function SaveAndUpdateMatFile_pushbutton_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SaveAndUpdateMatFile_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
