...
% Last Modified by GUIDE v2.5 10-Feb-2022 13:21:59
...
GuiGlobalsStruct.SectionListName = 'CoarseSectionList';
% answer = questdlg('Would you like to use the Yolo Section List?','Yolo Section List?','Yes','No','default');
% if strcmp(answer,'Yes')
%     GuiGlobalsStruct.IsDisplayYoloSectionList = 1;
%     GuiGlobalsStruct.SectionListName = 'YoloSectionList';
%     GuiGlobalsStruct.SectionList = GuiGlobalsStruct.YoloSectionList;
% elseif strcmp(answer,'No')
%     GuiGlobalsStruct.IsDisplayYoloSectionList = 0;
%     GuiGlobalsStruct.SectionListName = 'CoarseSectionList';
%     GuiGlobalsStruct.SectionList = GuiGlobalsStruct.CoarseSectionList;
% end
...
%             Y_Stage_mm = GuiGlobalsStruct.FullMapData.TopStageY_mm - 0.5*(GuiGlobalsStruct.FullMapData.TileFOV_microns/1000) + y_mouse_inFullResolutionPixels*NativePixelHeight_mm
        ...
        if IsAbort == false
            ...
            % here.
            if ~isfield(GuiGlobalsStruct,GuiGlobalsStruct.SectionListName)
...
% if strcmp(GuiGlobalsStruct.SectionListName,'CoarseSectionList')
%     GuiGlobalsStruct.CoarseSectionList = GuiGlobalsStruct.SectionList;
% else 
%     GuiGlobalsStruct.YoloSectionList = GuiGlobalsStruct.SectionList;
% end
...
autoOrderSections = get(handles.autoOrderSections_CheckBox,'value');

if autoOrderSections
...
% if ~GuiGlobalsStruct.IsDisplayYoloSectionList
%     GuiGlobalsStruct.CoarseSectionList = GuiGlobalsStruct.SectionList;
% else 
%     GuiGlobalsStruct.YoloSectionList = GuiGlobalsStruct.SectionList;
% end
...
%MatFileName =convertCharsToStrings(GuiGlobalsStruct.SectionListName);
 
%SectionListFileName = sprintf(append('%s\\'+MatFileName+'.mat'), GuiGlobalsStruct.FullWaferTileImagesDirectory);
SectionListFileName = sprintf('%s\\CoarseSectionList.mat', GuiGlobalsStruct.FullWaferTileImagesDirectory);
if exist(SectionListFileName, 'file')
    MyStr = sprintf('%s already exists. Manually erase then press again (saftey precatuion)', SectionListFileName);
    uiwait(msgbox(MyStr));
    return;
else
    CoarseSectionList = GuiGlobalsStruct.CoarseSectionList;
    save(SectionListFileName,'CoarseSectionList');
    MyStr = sprintf('Saved: %s, Close AutoMapGUI to go on to next step.', SectionListFileName);
    uiwait(msgbox(MyStr));
end
...
% --- Executes on button press in autoOrderSections_CheckBox.
function autoOrderSections_CheckBox_Callback(hObject, eventdata, handles)
% hObject    handle to autoOrderSections_CheckBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of autoOrderSections_CheckBox


% --- Executes on button press in Yolo.
% function Yolo_Callback(hObject, eventdata, handles)
% % hObject    handle to Yolo (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    structure with handles and user data (see GUIDATA)
% global GuiGlobalsStruct;
% %sz = [GuiGlobalsStruct.FullMapData.NumTileRows GuiGlobalsStruct.FullMapData.NumTileColumns];
% %tilesWithLabels = cell(sz);
% button = questdlg('Would you like to train the model again? choose No if you would like to use the pre-trained one.','the Yolo network','Yes','No','default');
% % Handle response
% load YoloDetector.mat yolov3Detector4;
% GuiGlobalsStruct.YoloDetector = yolov3Detector4;
% if strcmp(button,'Yes')
%     answer = questdlg('Do you have the groundTruth of the sections for train?','groundtruth','Yes','No','default');
%     if strcmp(answer,'Yes')
%         MyStr = sprintf('Upload the groundTruth.');
%         uiwait(msgbox(MyStr));
%         load(uigetfile);
%         %load uigetfile gTruth;
%     elseif strcmp(answer,'No')
%         MyStr = sprintf('Load the tiles of the first wafer in the ImageLabeler. Define new label with "section" name. Then, label the sections in each tile. At last, export the groundtruth as the gTruth.mat file.');
%         uiwait(msgbox(MyStr));
%         MyStr = sprintf('Select the path of the tiles for labeling.');
%         uiwait(msgbox(MyStr));
%         tilepath = uigetdir;
%         tiles = dir(fullfile(tilepath,'Tile*.tif')); 
%         mkdir(tilepath,'tilesForTraining'); 
%         trainpath = fullfile(tilepath,'tilesForTraining');
%         fileattrib(trainpath,'-w');
%         for i = 1:numel(tiles)
%             tile = imread(fullfile(tiles(i).folder,tiles(i).name));
%             tile = imresize(tile,[227,227]);
%             tile = imadjust(tile);
%             tile = cat(3,tile,tile,tile);
%             imwrite(tile,fullfile(trainpath,tiles(i).name));
%         end
%         imageLabeler(trainpath);
%     end
%     if ~exist('gTruth','var')
%         MyStr = sprintf('There is no gTruth variable. Make sure that you export the groudtruth of the sections to the workspace by the ImageLabeler.'); 
%         uiwait(msgbox(MyStr));
%         return;
%     end
%     trainingDataTable = objectDetectorTrainingData(gTruth);
%     imdsTrain = imageDatastore(trainingDataTable.imageFilename);
%     bldsTrain = boxLabelDatastore(trainingDataTable(:, 2:end));
%     trainingData = combine(imdsTrain, bldsTrain);
%     augmentedTrainingData = transform(trainingData, @augmentData);
%     preprocessedTrainingData = transform(augmentedTrainingData, @(data)preprocess(GuiGlobalsStruct.YoloDetector, data));
...
%     end
%     classNames = trainingDataTable.Properties.VariableNames(2:end);
%     mbqTrain = minibatchqueue(preprocessedTrainingData, 2,...
%         'MiniBatchSize', GuiGlobalsStruct.YoloParameters.MiniBatchSize,...
%         'MiniBatchFcn', @(images, boxes, labels) createBatchData(images, boxes, labels, classNames), ...
%         'MiniBatchFormat', ["SSCB", ""],...
%         'DispatchInBackground', dispatchInBackground,...
%         'OutputCast', ['', "double"]);
%     GuiGlobalsStruct.YoloParameters.doTraining = 1;
%     newYolov3Detector = trainYolo(GuiGlobalsStruct.YoloDetector,mbqTrain,GuiGlobalsStruct.YoloParameters);
%     answer = questdlg('Would you like to replace the primary model with this retrained one?','replacing the model','Yes','No','default');
%     if strcmp(answer,'Yes')
%         GuiGlobalsStruct.YoloDetector = newYolov3Detector;
%         MyStr = sprintf('the model replacement is done.'); 
%         uiwait(msgbox(MyStr));
%         return;
%     elseif strcmp(answer,'No')
%         return;
...
%     disp(['Please wait! it is detecting the sections in the tiles using the pretrained Yolo network.'])
%     p = GuiGlobalsStruct.FullWaferTileImagesDirectory;
%     tiles = dir(fullfile(p,'Tile*.tif'));    
%     bboxes = cell(numel(tiles),1);
%     scores = cell(numel(tiles),1);
%     labels = cell(numel(tiles),1);
%     sectionPoints = zeros(0,2);
%     for i = 1:numel(tiles)
%         k = findstr(tiles(i).name,'R');
%         row = str2num(tiles(i).name(k+1:k+3));
%         k = findstr(tiles(i).name,'C');
%         col = str2num(tiles(i).name(k+1:k+3));
%         tile = imread(fullfile(tiles(i).folder,tiles(i).name));
%         tile = imresize(tile,[227,227]);
%         x1 = (row - 1)*227+1;
%         x2 = row*227;
%         y1 = (col-1)*227+1;
%         y2 = col*227;
%         YoloFullImage(x1:x2 , y1:y2) = tile;
%         tile1 = imadjust(tile);
%         tile1 = cat(3,tile1,tile1,tile1);
%         [bboxes{i},scores{i},labels{i}] = detect(GuiGlobalsStruct.YoloDetector,tile1);
%         %tilesWithLabels{col,row} = insertObjectAnnotation(tile,'rectangle',bboxes{i},labels{i});
%         if ~isempty(bboxes{i})
%             for j=1:size(bboxes{i})
%                 sectionPoints (end+1,1) = bboxes{i}(j,1) + (col-1)*227 + round(bboxes{i}(j,3)/2);
%                 sectionPoints (end,2) = bboxes{i}(j,2) + (row-1)*227 + round(bboxes{i}(j,4)/2);
%             end
%         end
%     end
% end
% ratio_x = size(GuiGlobalsStruct.FullMapImage,1)/size(YoloFullImage,1);
% ratio_y = size(GuiGlobalsStruct.FullMapImage,2)/size(YoloFullImage,2);
% OriginalSectionPoints(:,1) = round(sectionPoints(:,1).*ratio_x);
% OriginalSectionPoints(:,2) = round(sectionPoints(:,2).*ratio_y);
% % [OriginalSectionPoints] = PostprocessAfterYoloSectionDetection(OriginalSectionPoints);
% imshow(GuiGlobalsStruct.FullMapImage);
% hold on;
% plot(OriginalSectionPoints(:,1), OriginalSectionPoints(:,2), 'r+', 'LineWidth', 0.5, 'MarkerSize', 7);
% 
% clear YoloSectionList;
% YoloSectionList(size(OriginalSectionPoints,1),1) = struct('rpeak',[],'cpeak',[],'Label',[],'IsDeleted',[]);
% for i=1:size(OriginalSectionPoints,1)
%     YoloSectionList(i).rpeak = OriginalSectionPoints(i,2);
%     YoloSectionList(i).cpeak= OriginalSectionPoints(i,1);
%     YoloSectionList(i).IsDeleted= false;
%      %GuiGlobalsStruct.YoloSectionList(i).Label= num2str(i);
% end
% GuiGlobalsStruct.YoloSectionList = YoloSectionList;
% 
% 
% 
% 
% % --- Executes on button press in trainParameters.
% function trainParameters_Callback(hObject, eventdata, handles)
% % hObject    handle to trainParameters (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    structure with handles and user data (see GUIDATA)
% open YoloTrainParameters.fig;
% 
% YoloTrainParameters
