...
OriginalImage_3xSizeWithFilledInBackground = double(zeros(3*HeightImage,3*WidthImage));
...
if 0 %Flatten contrast
    ...
    subplot(2,2,1)
    image(NewImage*256/max(NewImage(:)));
    subplot(2,2,2)
    image(OriginalImage_3xSizeWithFilledInBackground* 256/max(OriginalImage_3xSizeWithFilledInBackground(:)));
    
    
    OriginalImage_3xSizeWithFilledInBackground2 = OriginalImage_3xSizeWithFilledInBackground / max(OriginalImage_3xSizeWithFilledInBackground(:));
    OriginalImage_3xSizeWithFilledInBackground2 = OriginalImage_3xSizeWithFilledInBackground2 .^ flattenExp;
    
    NewImage2 = NewImage/max(NewImage(:));
    NewImage2 = NewImage2.^ flattenExp;
    
    subplot(2,2,3)
    image(NewImage2*256/max(NewImage2(:)));
    subplot(2,2,4)
    image(OriginalImage_3xSizeWithFilledInBackground2 * 256/max(OriginalImage_3xSizeWithFilledInBackground2(:)));
    
    
    NewImage = NewImage2;
    OriginalImage_3xSizeWithFilledInBackground = OriginalImage_3xSizeWithFilledInBackground2;
end
...
for i = 1:length(AnglesInDegreesToTryArray)
    ...
    if IsPlotResults
        ...
        ColorCombinedImage(:,:,1) = 0;
        ColorCombinedImage(HeightImage+1:(end-HeightImage), WidthImage+1:(end-WidthImage),1) = NewImage * 256/max(NewImage(:)); %red (centered template)
        ColorCombinedImage(:,:,2) = OriginalImage_3xSizeWithFilledInBackground_rotated * 256/max(OriginalImage_3xSizeWithFilledInBackground_rotated(:)); %green
        ColorCombinedImage(:,:,3) = 0*OriginalImage_3xSizeWithFilledInBackground_rotated; %blue
        
        figure(h_fig);
        subplot(1,3,1);
        image(uint8(ColorCombinedImage*3));
    end
    ...
    if IsPlotResults
        ...
        ColorCombinedImage(HeightImage+1+r_offset:(end-HeightImage)+r_offset,...
            WidthImage+1+c_offset:(end-WidthImage)+c_offset,1) = NewImage * 256/max(NewImage(:));
