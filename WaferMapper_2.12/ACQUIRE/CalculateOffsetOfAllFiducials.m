...
while true
    ...
    if exist(ReimagedFiducialFileNameStr, 'file')
        ...
        ReImagedImageDS = imresize(ReImagedImage,1/DSFactor,'bilinear');
          ...
          % add band pass joshm
         OriginalImageDS = mexHatFiducial(OriginalImageDS);
         ReImagedImageDS = mexHatFiducial(ReImagedImageDS);

        
        %START: Code to do different scales
        if IsCalculateFudgeScale
            
            if ~exist('BestScaleFactorFromFirstFiducial','var')  %Only run through different scales on first fiducial
