function Fibics_AcquireImage(ImageWidthInPixels, ImageHeightInPixels, DwellTimeInMicroseconds, FileNameStr,...
    FOV_microns, IsDoAutoRetakeIfNeeded, IsMagOverride, MagForOverride,  WaferNameStr, LabelStr, IsReturnImmediatly,LineAverage)
...
% 
% LineAverage = 10;
% if ~exist('LineAverage','var')
%     LineAverage = 1;
% end
% MyCZEMAPIClass.Set_PassedTypeSingle('DP_NOISE_REDUCTION',5);
% MyCZEMAPIClass.Set_PassedTypeSingle('AP_LINE_AVERAGE_COUNT',LineAverage);
        ...
        if IsDoAutoRetakeIfNeeded
            ...
            if (AvgOfBottom10PercentOfImage < 5) || (AvgOfBottom10PercentOfImage > 250) %reacquire image
                ...
                if goodMove
                    ...
                    MyCZEMAPIClass.Fibics_AcquireImage(ImageWidthInPixels,ImageHeightInPixels,...
                        DwellTimeInMicroseconds,FileNameStr);
%                     getLineAverage = MyCZEMAPIClass.Get_PassedTypeSingle('AP_LINE_AVERAGE_COUNT');
%                       disp(sprintf('line average is set to %d',getLineAverage))
                    while(MyCZEMAPIClass.Fibics_IsBusy)
                        pause(1);
                    end
                    pause(1);
