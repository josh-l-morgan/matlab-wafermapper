...
% Last Modified by GUIDE v2.5 30-Oct-2022 20:08:19
...
%one way to increase dlg box font size, not best solution
%GuiGlobalsStruct.DialogFontSize = 20;
%TeX interpreter to allow for font changes
%opts.Interpreter = 'tex';
%opts.Default = 'OK';
%myStr = sprintf("\\bf \\fontsize{%d}About to init Zeiss and FIBICS APIs. Press cancel to skip.", GuiGlobalsStruct.DialogFontSize);
%ButtonName = questdlg(myStr,'title','OK','Cancel',opts);
...
if strcmp(ButtonName,'OK')
    ...
    if GuiGlobalsStruct.IsZeissAPIInitialized
        ...
        if GuiGlobalsStruct.MyCZEMAPIClass.GetMag() == StartMag
            ...
            if GuiGlobalsStruct.MyCZEMAPIClass.Fibics_Initialise()
                MyStr = sprintf('Fibics API successfully initialized.');
            else
                MyStr = sprintf('Fibics API was notsuccessfully initialized.');
            end
            MyStr
            StatusBarMessage(MyStr);
...
GuiGlobalsStruct.MontageTarget.LowResForAlignWidthInMicrons = 200;
GuiGlobalsStruct.MontageTarget.LowResForAlignHeightInMicrons = 200;
...
%dark background and large text option setup
GuiGlobalsStruct.IsDarkBackground = false;
GuiGlobalsStruct.BackgroundColorValue = [0.94 0.94 0.94]; %light
GuiGlobalsStruct.FigureFontSize = 14; %adjustable desired font size for figure images (like wafer section numbers) 
GuiGlobalsStruct.DialogFontSize = 20; %adjustable desired font size for pop up dialogs
set(groot, 'DefaultTextFontSize', GuiGlobalsStruct.FigureFontSize);
set(groot, 'DefaultTextFontWeight', 'bold');
set(groot, 'DefaultAxesFontSize', GuiGlobalsStruct.FigureFontSize);
...
if ~exist(GuiGlobalsStruct.UTSLDirectory, 'dir')
    ...
    if exist(GuiGlobalsStruct.FullWaferTileImagesDirectory,'dir')
        ...
        if exist(FullMapDataFileNameStr,'file')
            ...
            fprintf('Loading FullMapData.mat file: %s\n',FullMapDataFileNameStr);
    ...
    if exist(GuiGlobalsStruct.FullWaferTileImagesDirectory,'dir')
        ...
        if exist(GuiGlobalsStruct.FullWaferTileImagesDirectory,'dir')
            ...
            if exist(CoarseSectionListFileNameStr,'file')
                fprintf('Loading CoarseSectionList.mat file: %s\n',CoarseSectionListFileNameStr);
...
fprintf('Turning stage backlash ON in X and Y\n');
...
if 0;%~IsJustOptical %if we are not doing optical only then go through normal process, else skip most
    ...
    if k == 0 %was mouse press
        ...
        fprintf('UL_R = %d, UL_C = %d\nLR_R = %d, LR_C = %d\n',UpLeftCorner_R,UpLeftCorner_C,LowerRightCorner_R,LowerRightCorner_C);
...
if strcmp(AnswerName,'No')
    ...
    AnswerOverview = questdlg('View example overview anyway?', 'Question', 'Yes', 'No', 'Yes');
...
[StageTransform, FudgeScaleUsedForLowRes] = CalculateOffsetOfAllFiducials(GuiGlobalsStruct.LowResFiducialsDirectory, GuiGlobalsStruct.ReimageLowResFiducialsDirectory, 0, 1)
...
for FiducialNum = 1:length(GuiGlobalsStruct.FiducialAlignmentArray)
    ...
    fprintf('Fiducial#%d: XOffset_Meters = %d, YOffset_Meters = %d\n',FiducialNum,XOffset_Meters,YOffset_Meters);
...
if length(ListOfTifFileNames) > 0
    [SelectionNumber,isok] = listdlg('PromptString','Select a wafer dir:',...
        'SelectionMode','single','ListString',ListOfTifFileNames,'ListSize',[200,300]);
...
fprintf('WaferName_InDisplay = %s\n', WaferName_InDisplay);

% %Determine what wafer is loaded

WaferName_CurrentlyLoaded = GuiGlobalsStruct.WaferName;
fprintf('WaferName_CurrentlyLoaded = %s\n', WaferName_CurrentlyLoaded);
...
if ~exist(GuiGlobalsStruct.ManuallyCorrectedStagePositionsDirectory, 'dir')
    ...
    fprintf('Creating directory: %s\n',GuiGlobalsStruct.ManuallyCorrectedStagePositionsDirectory);
...
if exist(DataFileNameStr)
    ...
    fprintf('Moving stage to(%0.5g, %0.5g)\n',StageX_Meters,StageY_Meters);
...
% --- Executes during object creation, after setting all properties.
function OpenMasterDirForUTSLs_MenuItem_CreateFcn(hObject, eventdata, handles)
% hObject    handle to OpenMasterDirForUTSLs_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --------------------------------------------------------------------
function AcquireLowHighFids_MenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to AcquireLowHighFids_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if exist(GuiGlobalsStruct.LowResFiducialsDirectory, 'dir')
    fprintf('%s already exists. Wafermapper will copy and backup the old directory.\n', GuiGlobalsStruct.LowResFiducialsDirectory);
    %uiwait(msgbox(MyStr,'modal'));
    [success,message,messageid] = mkdirBackup(GuiGlobalsStruct.LowResFiducialsDirectory);
else
    [success,message,messageid] = mkdir(GuiGlobalsStruct.LowResFiducialsDirectory);
...
MyStr = sprintf('LEFT CLICK in full wafer display to move stage to that position. \nRIGHT CLICK to ZOOM. \nPress ''G'' Key to grab fiducial image at current position. \nPress ESC key to finish');
uiwait(msgbox(MyStr));
getLowAndHighResFiducials;

set(handles.AcquireLowResFiducials_MenuItem,'Checked','on');
set(handles.AcquireHighResFiducials_MenuItem,'Checked','on'); %Next step
set(handles.AcquireLowHighFids_MenuItem,'Checked','on'); %Next step
set(handles.AcquireExampleSectionImage_MenuItem,'Enable','on');
set(handles.AcquireLowResFiducials_MenuItem,'Enable','on');
set(handles.AcquireHighResFiducials_MenuItem,'Enable','on'); %Next step


% --- Executes during object creation, after setting all properties.
function Axes_SectionOverviewDisplay_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Axes_SectionOverviewDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

% Hint: place code in OpeningFcn to populate Axes_SectionOverviewDisplay


% --- Executes during object creation, after setting all properties.
function Axes_FullWaferDisplay_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Axes_FullWaferDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

% Hint: place code in OpeningFcn to populate Axes_FullWaferDisplay
...
% --- Executes during object creation, after setting all properties.
function SectionDecrement_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SectionDecrement_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function GoToTargetPoint_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to GoToTargetPoint_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function SetupZoomParameters_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SetupZoomParameters_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function LockCurrentZoomParameters_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to LockCurrentZoomParameters_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function ResetZoomToFull_MenuItem_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ResetZoomToFull_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function SectionIncrement_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SectionIncrement_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function GoToTargetPointWithImageBasedCorrection_Button_CreateFcn(hObject, eventdata, handles)
% hObject    handle to GoToTargetPointWithImageBasedCorrection_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function QuickManualPanel_CreateFcn(hObject, eventdata, handles)
% hObject    handle to QuickManualPanel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called


% --------------------------------------------------------------------
function accessibilityMenu_Callback(hObject, eventdata, handles)
% hObject    handle to accessibilityMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --------------------------------------------------------------------
function darkBackground_Callback(hObject, eventdata, handles)
% hObject    handle to darkBackground (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    global GuiGlobalsStruct;
    if strcmp(get(hObject,'Checked'),'on')
        GuiGlobalsStruct.BackgroundColorValue = [0.94 0.94 0.94]; %light
        GuiGlobalsStruct.IsDarkBackground = 0;
        set(hObject,'Checked','off');
    else 
        GuiGlobalsStruct.BackgroundColorValue = [0.2 0.2 0.2]; %dark
        GuiGlobalsStruct.IsDarkBackground = 1;
        set(hObject,'Checked','on');
    end
    set(gcf, 'Color', GuiGlobalsStruct.BackgroundColorValue); %set current main window background color
