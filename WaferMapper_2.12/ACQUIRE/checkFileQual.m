...
toc
%%
dsamp = 512; %number of kernal samples in each dimension 
...
%grab samples
%%%%%%%%%%%READING FULL IMAGE may have to return to sub sampling %%%%%%%%%%

tic
dsp = fix((ys-5)/dsamp);
It = imread(checkFile);
Idown = ones(fix((size(It,1)-2)/dsp),fix((size(It,2)-2)/dsp));
[yds xds] = size(Idown);
[yi xi] = find(Idown);

Id = zeros(yds, xds, length(yshift));
for i = 1:length(yshift)
    fullInd = sub2ind(size(It),(yi - 1) * dsp +yshift(i) + 1,(xi - 1) * dsp +xshift(i) + 1);
    Idown(:) = It(fullInd);
    Id(:,:,i) = Idown;
end
'qual get data time'
