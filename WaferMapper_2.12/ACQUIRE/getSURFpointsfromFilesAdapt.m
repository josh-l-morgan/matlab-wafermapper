...
if (readRegion)
    ...
    data = filtDat(data,dSamp);


fp = figure;
pax = gca;
colormap(fp,gray(256))
for i = 1:10
...
for i=1:Z
    p=points{i};
    for k=1:length(p)
        p(k).x=(p(k).x *dSamp) +Cols(1)-1;
        p(k).y=(p(k).y * dSamp)+Rows(1)-1;
    end
    points{i}=p;
end
...
function[dat] = filtDat(data,dSamp)

dat = data;
dat = 256-dat;
dat = imresize(dat,1/dSamp);
dat = medfilt2(dat,[3,3]);
