...
%%REset working distance Karl dist fix ***
GuiGlobalsStruct.MontageParameters.WDResetThreshold = 0.01;%0.0005; jm changed arbitrarily
...
if (abs(startWD - lastWD) > WDResetThreshold)
    ...
    %calculation)
    disp('Difference from last working distance is too big')
...
if targetFocus %figure out what's going on here %chch
    ...
    satF = imgaussfilt(double(satD),2);
    IdeSat = I;
    IdeSat(satD) = 0;
    
    %%Difference of gaussian
    Ig1 = imgaussfilt(IdeSat,.5);
    Ig2 = imgaussfilt(I,.6);
    IgD = Ig1-Ig2;
    IgD(IgD<0) = 0;
    IgD = IgD*255/max(IgD(:));
    ...
    if isempty(y)
        y = round(size(fIdomed,1)/2);
        x = round(size(fIdomed,2)/2);
    end
    
    %Karl is moving the spot manually a little bit adjacent
    if y+10 < ImageHeightInPixels/2
        y=y+10;
    else
        y=y-10;
    end
    if x+10 < ImageHeightInPixels/2
        x=x+10;
    else
        x=x+10;
    end
    ...
    subplot(2,1,1)
    image(fIdomed*100/mean(fIdomed(:))),pause(.01)
    hold on
    scatter(x,y,'x','r')
    hold off
    subplot(2,1,2)
...
if IsPerformAutoStig
    ...
    for repeatStig = 1:1
        ...
        if (abs(AutoWorkingDistance - CurrentWorkingDistance) > WDResetThreshold)
            ...
            %AFStartingWD, which is the WD used at the beginning of this
            %calculation)
            disp('There seems to have been a big change in working distance')
            sm.Set_PassedTypeSingle('AP_WD', CurrentWorkingDistance);
        ...
        sm.Set_PassedTypeSingle('AP_STIG_Y',startStigY);
        pause(.1)
        sm.Set_PassedTypeSingle('AP_Mag',1000);%StartingMagForAF / repeatStig);
    ...
    if (abs(AutoWorkingDistance - CurrentWorkingDistance) > WDResetThreshold)
        ...
        %(=AFStartingWD)
        disp('Autofunctions have made changes that excede the allowed working distance change threshold')
