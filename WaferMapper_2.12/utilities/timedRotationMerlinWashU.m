...
elseif 0
X =  0.064; %0 is limit. 50 still causing faults with stage bias off
Y =  0.0635; %130 is limit, 98 still causing faults with stage bias off
Z =  0.025;

elseif 1 % set stage to lowest position in the middle xy
...
sm.Set_PassedTypeSingle('AP_STAGE_GOTO_T',0);
smwait(sm,'DP_STAGE_IS');
...
sm.Set_PassedTypeSingle('AP_STAGE_GOTO_R',oldR);
smwait(sm,'DP_STAGE_IS');
